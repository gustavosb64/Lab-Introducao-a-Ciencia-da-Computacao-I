...
Faça um programa em C que rode uma simulação com as regras descritas abaixo. Para isso, uma matriz de 32 linhas e 64 colunas deve ser criada, onde cada elemento é do tipo char. A cada frame essa matriz deve ser impressa na tela, isso é, cada linha impressa como uma string e ao final dela um caractere '\n' para indicar o final da linha. Após isso, uma função de aplicar física será executada. Essa função atualizará a matriz para ser impressa no próximo frame.
...
Com algumas adições no código, é possível tornar as partículas coloridas e renderizar a simulação no terminal de maneira mais fácil de visualizar.
